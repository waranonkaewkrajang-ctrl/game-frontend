...
export default function GameSettingsPage() {
  ...
  return (
    <div style={{ maxWidth: "700px" }}>
      ...
      <p style={{ color: "#64748b", fontSize: "0.85rem", marginBottom: "1.5rem" }}>กำหนด % คืนยอดเสีย, % ค่าคอมแนะนำเพื่อน และวันหมดอายุรางวัล</p>
      ...
      <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: "0.75rem", padding: "1.25rem" }}>
          ...
          <p style={{ fontSize: "0.8rem", color: "#64748b", margin: "0 0 1rem" }}>ระบบจะคำนวณยอดเสียรายวัน แล้วเก็บเป็นรางวัลรอลูกค้ากดรับ</p>
          <div>
        ...
        <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: "0.75rem", padding: "1.25rem" }}>
          ...
          <p style={{ fontSize: "0.8rem", color: "#64748b", margin: "0 0 1rem" }}>คิดค่าคอมจากยอดเสียของคนที่ถูกแนะนำ คำนวณรายวันพร้อม cashback</p>
          <div>
          ...
          <div style={{ marginTop: "0.75rem", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "0.5rem", padding: "0.75rem" }}>
            <p style={{ fontSize: "0.78rem", color: "#1d4ed8", margin: 0 }}>ตัวอย่าง: เพื่อนเสีย 1,000 บาท → ผู้แนะนำได้ {(1000 * parseFloat(settings.referral_percent || "0") / 100).toFixed(2)} บาท</p>
          </div>
        </div>

        {/* วันหมดอายุรางวัล */}
        <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: "0.75rem", padding: "1.25rem" }}>
          <h3 style={{ fontSize: "0.95rem", fontWeight: 700, color: "#0f172a", margin: "0 0 0.5rem" }}>วันหมดอายุรางวัล</h3>
          <p style={{ fontSize: "0.8rem", color: "#64748b", margin: "0 0 1rem" }}>กำหนดว่ารางวัล (ยอดเสีย + ค่าแนะนำ) รับย้อนหลังได้กี่วัน เกินกำหนดจะหมดอายุอัตโนมัติ</p>
          <div>
            <label style={labelStyle}>จำนวนวัน (0 = ไม่หมดอายุ)</label>
            <input className="input" type="number" step="1" min="0" max="365" value={settings.reward_expire_days} onChange={(e) => setSettings({ ...settings, reward_expire_days: e.target.value })} placeholder="เช่น 3" />
          </div>
          <div style={{ marginTop: "0.75rem", background: parseInt(settings.reward_expire_days || "0") > 0 ? "#fefce8" : "#f0fdf4", border: `1px solid ${parseInt(settings.reward_expire_days || "0") > 0 ? "#fde68a" : "#bbf7d0"}`, borderRadius: "0.5rem", padding: "0.75rem" }}>
            <p style={{ fontSize: "0.78rem", color: parseInt(settings.reward_expire_days || "0") > 0 ? "#854d0e" : "#166534", margin: 0 }}>
              {parseInt(settings.reward_expire_days || "0") > 0
                ? `ลูกค้าต้องกดรับภายใน ${settings.reward_expire_days} วัน ไม่งั้นรางวัลจะหมดอายุ`
                : "ไม่มีวันหมดอายุ ลูกค้ากดรับได้ตลอด"}
            </p>
          </div>
        </div>
